...
int kpayload(struct thread *td, struct kpayload_args* args){
	...
	void** got_prison0 =   (void**)&kernel_ptr[0x111B840];
	void** got_rootvnode = (void**)&kernel_ptr[0x21EBF20];

	//Resolve kernel functions...
	int (*copyout)(const void *kaddr, void *uaddr, size_t len) = (void *)(kernel_base + 0x271230);
	int (*printfkernel)(const char *fmt, ...) = (void *)(kernel_base + 0x0B79E0);
	...
	kernel_ptr[0x29A5F0] = 3; //9.03 pstate when shutdown

	//Kexec init
	void *DT_HASH_SEGMENT = (void *)(kernel_base+ 0xB4E1C0); // I know it's for 4.55 but I think it will works
	memcpy(DT_HASH_SEGMENT, kexec_data, kexec_size);

	void (*kexec_init)(void *, void *) = DT_HASH_SEGMENT;

	kexec_init((void *)(kernel_base+0x0B79E0), NULL);

	// Say hello and put the kernel base in userland to we can use later
	printfkernel("PS4 Linux Loader for 9.03 by valentinbreiz\n");
